...
create_buffer(app *app, off_t size)
{
...
static struct buffer *
create_buffer(app *app, int32_t stride, int32_t height, int32_t width,
...
main(void)
{
  ...
  struct zgn_cuboid_window *cuboid_window = zgn_shell_get_cuboid_window(
      app.shell, virtual_object, &half_size_array, &quaternion_array);
